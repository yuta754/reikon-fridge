...
@Controller
public class SelectFoodUpdateFoodController {

	@Autowired
	SearchFoodGenreService genre;
	
	@Autowired
	SearchFoodService food;
	...
	@GetMapping({"/selectFoodUpdateFood"})
	public String updateFoodInput(@RequestParam("id") Integer foodId, Model model) {
		//食材ジャンルテーブルを全件取得
		model.addAttribute("food_genre", genre.findAll());
	    ...
	    //食材情報の取得
		List<Food> foodInfo = food.findById(foodId);
